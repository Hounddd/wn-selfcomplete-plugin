 ...
 * @param {array} arrValues Array of possible values
 */
window.selfcomplete = function(inputEl, arrValues) {
    ...
    function createList(inputEl) {
        ...
        // Reduce the
        listeValues = arrValues.filter((key) => key.toUpperCase().includes(val.toUpperCase()));
        ...
        for (i = 0; i < listeValues.length; i++) {
            ...
            if (val != '' && listeValues[i].toUpperCase().includes(val.toUpperCase())) {
                // Highlight matching letters in suggestion
                matchNode.innerHTML = listeValues[i].replace(new RegExp(val, "gi"), function (match) {
                    return '<strong>' + match + '</strong>'
                });
            } else {
                matchNode.innerHTML = listeValues[i];
            }

            // Insert a input field that will hold the current suggestion value
            matchNode.innerHTML += "<input type='hidden' value='" + listeValues[i] + "'>";
    ...
    // Add global docuement event listener to close all suggestions lists
